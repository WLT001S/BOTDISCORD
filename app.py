...
TELEGRAM_TOKEN = "7598376591:AAF3wV6zMgIRfZIYtd71YcmZWbImcbpDOHE"
CHAT_ID = "6072481570"
...
def send_telegram_menu(text, buttons):
    reply_markup = {
...
def get_telegram_text_response(prompt="Hãy nhập dữ liệu:"):
    send_telegram_message(prompt)
    last_update_id = None
    while True:
        res = requests.get(f"{API_URL}/getUpdates").json()
        for result in res.get("result", []):
            message = result.get("message", {})
            if message and str(message.get("chat", {}).get("id")) == CHAT_ID:
                if "text" in message:
                    requests.get(f"{API_URL}/getUpdates?offset={result['update_id'] + 1}")
                    return message["text"].strip()
        time.sleep(2)

def wait_for_button(prompt, options):
    send_telegram_menu(prompt, options)
    while True:
        res = requests.get(f"{API_URL}/getUpdates").json()
        for result in res.get("result", []):
            callback = result.get("callback_query")
            if callback and str(callback["message"]["chat"]["id"]) == CHAT_ID:
                data = callback["data"]
                requests.get(f"{API_URL}/getUpdates?offset={result['update_id'] + 1}")
                return data
        time.sleep(2)
...
def run_tool_via_telegram():
    # Cài đặt thư viện nếu cần
    install = wait_for_button("Bạn có muốn cài thư viện không?", ["y", "n"])
    if install == "y":
        try:
            import pip
            _pkgs = ["requests", "colorama"]
            subprocess.check_call([sys.executable, "-m", "pip", "install"] + _pkgs)
            send_telegram_message("✓ Cài đặt thành công!")
        except Exception as e:
            send_telegram_message("✗ Lỗi cài đặt: " + str(e))
            return

    user = get_telegram_text_response("B1: Gửi Link hoặc Username:")
    key = get_telegram_text_response("B2: Nhập ký tự xác nhận:")
    send_icon = wait_for_button("B3: Gửi icon không?", ["y", "n"])

    # Truyền vào tool
    send_telegram_message("B4: Đang chạy tool...")
    try:
        exec(open("bot.py.py", encoding="utf-8").read(), globals())
    except Exception as e:
        send_telegram_message("Lỗi khi chạy tool: " + str(e))

    action = wait_for_button("Hoàn tất. Bạn muốn làm gì tiếp?", ["Bắt đầu lại", "Thoát"])
    if action == "Bắt đầu lại":
        run_tool_via_telegram()
    else:
        send_telegram_message("Tạm biệt!")

def main():
    send_telegram_message("Bot Telegram đã khởi động.")
    main_choice = wait_for_button("Chào mừng! Bấm Bắt đầu để chạy tool.", ["Bắt đầu", "Thoát"])
    if main_choice == "Bắt đầu":
        run_tool_via_telegram()
    else:
        send_telegram_message("Bot kết thúc.")
